...
func (s *RBACService) AddNewPermission(name, description string, employeeID int) (int, error) {
	var existing Permission
	if err := s.db.Where("name = ?", name).First(&existing).Error; err == nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "add_permission", fmt.Sprintf("permission:%s", name), false)
		}
	...
	s.invalidateCache("permissions:all")
	if s.auditEnabled {
		s.AuditLog(employeeID, "add_permission", fmt.Sprintf("permission:%s", name), true)
	}
	return perm.ID, nil
}

// GetAllPermissionsByRoleName retrieves all permission names for a role
func (s *RBACService) GetAllPermissionsByRoleName(roleName string, employeeID int) ([]string, error) {
	...
	if err := s.redisClient.Get(ctx, cacheKey).Scan(&permissions); err == nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "get_role_permissions", fmt.Sprintf("role:%s", roleName), true)
		}
	...
	if err := s.db.Where("name = ?", roleName).First(&role).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "get_role_permissions", fmt.Sprintf("role:%s", roleName), false)
		}
	...
	if err := s.db.Where("permission_id IN ?", permIDs).Find(&perms).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "get_role_permissions", fmt.Sprintf("role:%s", roleName), false)
		}
...
func (s *RBACService) AddNewRole(name, description string, permissionNames []string, employeeID int) (int, error) {
	var existing Role
	if err := s.db.Where("name = ?", name).First(&existing).Error; err == nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "add_role", fmt.Sprintf("role:%s", name), false)
		}
	...
	s.invalidateCache("roles:all")
	s.invalidateCache("role_permissions:all")
	if s.auditEnabled {
		s.AuditLog(employeeID, "add_role", fmt.Sprintf("role:%s", name), true)
	}
	return role.ID, nil
}

// ModifyRolePermissions updates the permissions for a role
func (s *RBACService) ModifyRolePermissions(roleID int, permissionNames []string, employeeID int) error {
	var role Role
	if err := s.db.First(&role, roleID).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "modify_role_permissions", fmt.Sprintf("role:%d", roleID), false)
		}
	...
	s.invalidateCache("role_permissions:all")
	if s.auditEnabled {
		s.AuditLog(employeeID, "modify_role_permissions", fmt.Sprintf("role:%d", roleID), true)
	}
	return nil
}

// GetRoleIDByRoleName retrieves role ID by name
func (s *RBACService) GetRoleIDByRoleName(roleName string, employeeID int) (int, error) {
	...
	if err := s.redisClient.Get(ctx, cacheKey).Scan(&roleID); err == nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "get_role_id", fmt.Sprintf("role:%s", roleName), true)
		}
		return roleID, nil
	}
	...
	if err := s.db.Where("name = ?", roleName).First(&role).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "get_role_id", fmt.Sprintf("role:%s", roleName), false)
		}
...
func (s *RBACService) GetRoleNameByRoleID(roleID int, employeeID int) (string, error) {
	...
	if err := s.redisClient.Get(ctx, cacheKey).Scan(&roleName); err == nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "get_role_name", fmt.Sprintf("role:%d", roleID), true)
		}
		return roleName, nil
	}
	...
	if err := s.db.First(&role, roleID).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "get_role_name", fmt.Sprintf("role:%d", roleID), false)
		}
		return "", fmt.Errorf("role ID %d not found", roleID)
	}
...
func (s *RBACService) AssignRolesToEmployeeByNames(employeeID int, roleNames []string, actorID int) error {
	var roleIDs []int
	for _, roleName := range roleNames {
		roleID, err := s.GetRoleIDByRoleName(roleName, actorID)
		if err != nil {
			if s.auditEnabled {
				s.AuditLog(actorID, "assign_roles_by_names", fmt.Sprintf("employee:%d", employeeID), false)
			}
			return err
		}
		roleIDs = append(roleIDs, roleID)
	}
	err := s.AssignRolesToEmployee(employeeID, roleIDs, actorID)
	if err != nil {
		if s.auditEnabled {
			s.AuditLog(actorID, "assign_roles_by_names", fmt.Sprintf("employee:%d", employeeID), false)
		}
	...
	if s.auditEnabled {
		s.AuditLog(actorID, "assign_roles_by_names", fmt.Sprintf("employee:%d", employeeID), true)
	}
	return nil
}

// AssignRolesToEmployee assigns roles to an employee by role IDs
func (s *RBACService) AssignRolesToEmployee(employeeID int, roleIDs []int, actorID int) error {
	for _, roleID := range roleIDs {
		var role Role
		if err := s.db.First(&role, roleID).Error; err != nil {
			if s.auditEnabled {
				s.AuditLog(actorID, "assign_roles", fmt.Sprintf("employee:%d", employeeID), false)
			}
			return fmt.Errorf("role ID %d not found", roleID)
		}
	...
	s.invalidateCache("employee_roles:all")
	if s.auditEnabled {
		s.AuditLog(actorID, "assign_roles", fmt.Sprintf("employee:%d", employeeID), true)
	}
	return nil
}

// RemoveRolesFromEmployee removes roles from an employee
func (s *RBACService) RemoveRolesFromEmployee(employeeID int, roleIDs []int, actorID int) error {
	if err := s.db.Where("employee_id = ? AND role_id IN ?", employeeID, roleIDs).Delete(&EmployeeRole{}).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(actorID, "remove_roles", fmt.Sprintf("employee:%d", employeeID), false)
		}
	...
	s.invalidateCache("employee_roles:all")
	if s.auditEnabled {
		s.AuditLog(actorID, "remove_roles", fmt.Sprintf("employee:%d", employeeID), true)
	}
	return nil
}

// GetEmployeeRoles retrieves role IDs for an employee
func (s *RBACService) GetEmployeeRoles(employeeID int, actorID int) ([]int, error) {
	...
	if err := s.redisClient.Get(ctx, cacheKey).Scan(&roleIDs); err == nil {
		if s.auditEnabled {
			s.AuditLog(actorID, "get_employee_roles", fmt.Sprintf("employee:%d", employeeID), true)
		}
		return roleIDs, nil
	}

	var employeeRoles []EmployeeRole
	if err := s.db.Where("employee_id = ?", employeeID).Find(&employeeRoles).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(actorID, "get_employee_roles", fmt.Sprintf("employee:%d", employeeID), false)
		}
...
func (s *RBACService) GetEmployeeRoleNames(employeeID int, actorID int) ([]string, error) {
	roleIDs, err := s.GetEmployeeRoles(employeeID, actorID)
	if err != nil {
		if s.auditEnabled {
			s.AuditLog(actorID, "get_employee_role_names", fmt.Sprintf("employee:%d", employeeID), false)
		}
		return nil, err
	}
	...
	for _, roleID := range roleIDs {
		name, err := s.GetRoleNameByRoleID(roleID, actorID)
		if err != nil {
			if s.auditEnabled {
				s.AuditLog(actorID, "get_employee_role_names", fmt.Sprintf("employee:%d", employeeID), false)
			}
	...
	if s.auditEnabled {
		s.AuditLog(actorID, "get_employee_role_names", fmt.Sprintf("employee:%d", employeeID), true)
	}
	return roleNames, nil
}

// ValidatePermission checks if an employee has a specific permission
func (s *RBACService) ValidatePermission(employeeID int, requiredPermission string) error {
	...
	if err := s.redisClient.Get(ctx, cacheKey).Scan(&permissions); err == nil {
		for _, perm := range permissions {
			if perm == requiredPermission {
				if s.auditEnabled {
					s.AuditLog(employeeID, "validate_permission", fmt.Sprintf("permission:%s", requiredPermission), true)
				}
				return nil
			}
		}
	...
	roleIDs, err := s.GetEmployeeRoles(employeeID, employeeID)
	if err != nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "validate_permission", fmt.Sprintf("permission:%s", requiredPermission), false)
		}
		return err
	}
...
func (s *RBACService) ValidateAnyPermission(employeeID int, required []string) error {
	for _, perm := range required {
		if err := s.ValidatePermission(employeeID, perm); err == nil {
			if s.auditEnabled {
				s.AuditLog(employeeID, "validate_any_permission", fmt.Sprintf("permissions:%v", required), true)
			}
			return nil
		}
	}
	if s.auditEnabled {
		s.AuditLog(employeeID, "validate_any_permission", fmt.Sprintf("permissions:%v", required), false)
	}
...
func (s *RBACService) ValidateAllPermissions(employeeID int, required []string) error {
	for _, perm := range required {
		if err := s.ValidatePermission(employeeID, perm); err != nil {
			if s.auditEnabled {
				s.AuditLog(employeeID, "validate_all_permissions", fmt.Sprintf("permissions:%v", required), false)
			}
			return err
		}
	}
	if s.auditEnabled {
		s.AuditLog(employeeID, "validate_all_permissions", fmt.Sprintf("permissions:%v", required), true)
	}
	return nil
}

// CompareEmpByRoleName checks if an employee has a specific role
func (s *RBACService) CompareEmpByRoleName(employeeID int, roleName string) error {
	roleID, err := s.GetRoleIDByRoleName(roleName, employeeID)
	if err != nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "compare_by_role", fmt.Sprintf("role:%s", roleName), false)
		}
		return err
	}
	...
	for _, id := range roleIDs {
		if id == roleID {
			if s.auditEnabled {
				s.AuditLog(employeeID, "compare_by_role", fmt.Sprintf("role:%s", roleName), true)
			}
			return nil
		}
	}
...
func (s *RBACService) ValidatePermissionsBulk(employeeIDs []int, requiredPermission string, actorID int) (map[int]bool, error) {
	results := make(map[int]bool, len(employeeIDs))

	ctx := context.Background()
	var employeeRoles []EmployeeRole
	if err := s.db.Where("employee_id IN ?", employeeIDs).Find(&employeeRoles).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(actorID, "validate_permissions_bulk", fmt.Sprintf("permission:%s", requiredPermission), false)
		}
		return nil, fmt.Errorf("failed to fetch employee roles: %w", err)
	}
	...
	var perm Permission
	if err := s.db.Where("name = ?", requiredPermission).First(&perm).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(actorID, "validate_permissions_bulk", fmt.Sprintf("permission:%s", requiredPermission), false)
		}
	...
	if s.auditEnabled {
		s.AuditLog(actorID, "validate_permissions_bulk", fmt.Sprintf("permission:%s", requiredPermission), true)
	}
	return results, nil
}

// RefreshAllCaches refreshes all caches
func (s *RBACService) RefreshAllCaches(employeeID int) error {
	s.cacheMu.Lock()
	...
	if s.auditEnabled {
		s.AuditLog(employeeID, "refresh_caches", "all_caches", true)
	}
	return nil
}
...
func (s *RBACService) GetCacheStats(employeeID int) map[string]interface{} {
	...
	if s.auditEnabled {
		s.AuditLog(employeeID, "get_cache_stats", "cache_stats", true)
	}
	return s.cacheStats
}

// GetCacheMemoryUsage estimates cache memory usage
func (s *RBACService) GetCacheMemoryUsage(employeeID int) (map[string]int64, error) {
	keys := []string{
	...
	for _, key := range keys {
		...
		if err != nil && err != redis.Nil {
			if s.auditEnabled {
				s.AuditLog(employeeID, "get_cache_memory_usage", fmt.Sprintf("cache:%s", key), false)
			}
	...
	if s.auditEnabled {
		s.AuditLog(employeeID, "get_cache_memory_usage", "cache_memory", true)
	}
	return result, nil
}
...
func (s *RBACService) RbacMiddleware(permission string) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if !ok {
			if s.auditEnabled {
				s.AuditLog(0, "middleware_check", fmt.Sprintf("permission:%s", permission), false)
			}
			return fiber.NewError(fiber.StatusUnauthorized, "employee_id not found in context")
		}
		if err := s.ValidatePermission(employeeID, permission); err != nil {
			if s.auditEnabled {
				s.AuditLog(employeeID, "middleware_check", fmt.Sprintf("permission:%s", permission), false)
			}
			return fiber.NewError(fiber.StatusForbidden, err.Error())
		}
		if s.auditEnabled {
			s.AuditLog(employeeID, "middleware_check", fmt.Sprintf("permission:%s", permission), true)
		}
		return c.Next()
	}
}
...
func (s *RBACService) GetEmployeePermissions(employeeID int, actorID int) ([]string, error) {
	cacheKey := fmt.Sprintf("%semployee:%d:permissions", s.cachePrefix, employeeID)
	var permissions []string
	...
	if err := s.redisClient.Get(ctx, cacheKey).Scan(&permissions); err == nil {
		if s.auditEnabled {
			s.AuditLog(actorID, "get_employee_permissions", fmt.Sprintf("employee:%d", employeeID), true)
		}
		return permissions, nil
	}
	...
	if err != nil {
		if s.auditEnabled {
			s.AuditLog(actorID, "get_employee_permissions", fmt.Sprintf("employee:%d", employeeID), false)
		}
		return nil, err
	}
...
func (s *RBACService) DeletePermission(permissionName string, employeeID int) error {
	var perm Permission
	if err := s.db.Where("name = ?", permissionName).First(&perm).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "delete_permission", fmt.Sprintf("permission:%s", permissionName), false)
		}
		return fmt.Errorf("permission %s not found", permissionName)
	}

	if err := s.db.Where("permission_id = ?", perm.ID).Delete(&RolePermission{}).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "delete_permission", fmt.Sprintf("permission:%s", permissionName), false)
		}
	...
	s.invalidateCache("role_permissions:all")
	if s.auditEnabled {
		s.AuditLog(employeeID, "delete_permission", fmt.Sprintf("permission:%s", permissionName), true)
	}
	return nil
}

// DeleteRole removes a role and updates related mappings
func (s *RBACService) DeleteRole(roleName string, employeeID int) error {
	var role Role
	if err := s.db.Where("name = ?", roleName).First(&role).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "delete_role", fmt.Sprintf("role:%s", roleName), false)
		}
		return fmt.Errorf("role %s not found", roleName)
	}

	if err := s.db.Where("role_id = ?", role.ID).Delete(&RolePermission{}).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "delete_role", fmt.Sprintf("role:%s", roleName), false)
		}
		return fmt.Errorf("failed to delete role-permission mappings: %w", err)
	...
	if err := s.db.Where("role_id = ?", role.ID).Delete(&EmployeeRole{}).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "delete_role", fmt.Sprintf("role:%s", roleName), false)
		}
	...
	s.invalidateCache("employee_roles:all")
	if s.auditEnabled {
		s.AuditLog(employeeID, "delete_role", fmt.Sprintf("role:%s", roleName), true)
	}
	return nil
}

// ListAllPermissions retrieves all permissions in the system
func (s *RBACService) ListAllPermissions(employeeID int) ([]Permission, error) {
	...
	if err := s.redisClient.Get(ctx, cacheKey).Scan(&permissions); err == nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "list_permissions", "all_permissions", true)
		}
		return permissions, nil
	}
	...
	if err := s.db.Find(&permissions).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "list_permissions", "all_permissions", false)
		}
		return nil, fmt.Errorf("failed to fetch permissions: %w", err)
	}
...
func (s *RBACService) ListAllRoles(employeeID int) ([]Role, error) {
	...
	if err := s.redisClient.Get(ctx, cacheKey).Scan(&roles); err == nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "list_roles", "all_roles", true)
		}
		return roles, nil
	}
	...
	if err := s.db.Find(&roles).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "list_roles", "all_roles", false)
		}
...
func (s *RBACService) GetEmployeesByRoleName(roleName string, employeeID int) ([]int, error) {
	roleID, err := s.GetRoleIDByRoleName(roleName, employeeID)
	if err != nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "get_employees_by_role", fmt.Sprintf("role:%s", roleName), false)
		}
		return nil, err
	}
	...
	if err := s.redisClient.Get(ctx, cacheKey).Scan(&employeeIDs); err == nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "get_employees_by_role", fmt.Sprintf("role:%s", roleName), true)
		}
		return employeeIDs, nil
	}

	var employeeRoles []EmployeeRole
	if err := s.db.Where("role_id = ?", roleID).Find(&employeeRoles).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(employeeID, "get_employees_by_role", fmt.Sprintf("role:%s", roleName), false)
		}
		return nil, fmt.Errorf("failed to fetch employee roles: %w", err)
...
func (s *RBACService) AuditLog(employeeID int, action, resource string, success bool) error {
	if !s.auditEnabled {
		return nil
	}
...
func (s *RBACService) GetAuditLogs(employeeID *int, action *string, startTime, endTime *time.Time, actorID int) ([]map[string]interface{}, error) {
	query := s.db.WithContext(context.Background()).Model(&Audit{})
	if employeeID != nil {
	...
	if err := query.Find(&audits).Error; err != nil {
		if s.auditEnabled {
			s.AuditLog(actorID, "get_audit_logs", "audit_logs", false)
		}
	...
	if s.auditEnabled {
		s.AuditLog(actorID, "get_audit_logs", "audit_logs", true)
	}
	return results, nil
}

// invalidateCache invalidates a specific cache key
func (s *RBACService) invalidateCache(key string) {
	s.cacheMu.Lock()
	defer s.cacheMu.Unlock()
	ctx := context.Background()
	s.redisClient.Del(ctx, s.cachePrefix+key)
